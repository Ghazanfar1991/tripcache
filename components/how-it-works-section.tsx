...
const steps = [
  {
    ...
    description:
      "Simply forward any flight, hotel, or car rental confirmation to your TripCache email address.",
  ...
  },
  {
    ...
    description:
      "Our intelligent parser instantly extracts every detail and builds a beautiful, interactive itinerary.",
  ...
  },
  {
    ...
    description:
      "Access everything offline, track live flight updates, and export expense reports with a tap.",
...
export function HowItWorksSection() {
  return (
    <section
      id="how-it-works"
      className="relative overflow-hidden bg-background py-6 dark:bg-slate-950 sm:py-12"
    >
      {/* Background Decor */}
      ...
      <SectionContainer className="relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20 space-y-6">
          <div className="inline-flex items-center gap-2 rounded-full border border-border/70 bg-background/70 px-4 py-2 text-sm font-semibold uppercase tracking-wide text-muted-foreground shadow-lg backdrop-blur-md dark:border-white/10 dark:bg-white/5 dark:text-slate-300">
            How It Works
          </div>
          <h2 className="text-4xl font-extrabold tracking-tight text-foreground sm:text-5xl lg:text-6xl dark:text-white">
            The Travel{" "}
          ...
          <p className="mx-auto max-w-2xl text-lg leading-relaxed text-muted-foreground dark:text-slate-400">
            Three simple steps to transform messy confirmation emails into a
            master itinerary.
          </p>
        </div>
            ...
            {steps.map((step, index) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <div
                      className={`inline-flex items-center justify-center p-4 rounded-2xl bg-gradient-to-br ${step.gradient} shadow-2xl shadow-black/50 mb-4`}
                    >
                      <step.icon className="w-8 h-8 text-white" />
                    </div>
                    <div>
                      <h3 className="mb-2 text-sm font-bold uppercase tracking-widest text-muted-foreground dark:text-slate-500">
                        Step 0{index + 1}
                      </h3>
                      <h4
                        className={`text-3xl sm:text-4xl font-bold mb-4 text-transparent bg-clip-text bg-gradient-to-r ${step.textGradient}`}
                      >
                        {step.title}
                      </h4>
                  ...
                  <div className="absolute left-1/2 top-1/2 z-20 hidden h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-border bg-background dark:border-white/20 dark:bg-slate-950 lg:block">
                    <div
                      className={`absolute inset-0 rounded-full bg-gradient-to-tr ${step.gradient} blur-sm opacity-50`}
                    />
                  </div>

                  {/* Image Presentation */}
                  <div className="relative z-10 flex-1 w-full perspective-1000">
                    {/* Glow behind image */}
                    <div
                      className={`absolute inset-0 ${step.glow} blur-[80px] rounded-full -z-10 bg-opacity-40`}
                    />
