...
export function DesignOneHome() {
  return <main className="journal-paper min-h-screen overflow-hidden text-[#29251f] [font-family:ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,'Segoe_UI',sans-serif]">
    <section className="relative mx-auto grid min-h-screen max-w-[1380px] items-center gap-5 px-5 pb-12 pt-28 sm:px-8 sm:pt-28 min-[940px]:min-h-[100svh] min-[940px]:grid-cols-[minmax(0,1.02fr)_minmax(27rem,0.98fr)] min-[940px]:gap-10 min-[940px]:px-12 min-[940px]:pb-10 min-[940px]:pt-24">
      <div className="relative z-10 mx-auto max-w-[650px] py-3 text-center min-[940px]:mx-0 min-[940px]:py-8 min-[940px]:text-start">
        <Reveal><Eyebrow><Plane className="h-3.5 w-3.5" /> Travel itinerary organizer app</Eyebrow></Reveal>
        <Reveal delay={70}><h1 className="mt-6 design-one-display-home">Turn booking emails into one organized travel itinerary.</h1></Reveal>
        <Reveal delay={150}><p className="mx-auto mt-6 max-w-[590px] text-base leading-7 tracking-[-0.01em] text-[#696158] sm:text-lg sm:leading-8 min-[940px]:mx-0">Forward flight, hotel, car, tour, and ticket confirmations to TripCache. Review your itinerary, track free-cancellation deadlines, and keep documents, receipts, and expenses together.</p></Reveal>
        <Reveal delay={220} className="mt-7 flex justify-center min-[940px]:justify-start"><DesignOneStoreButtons /></Reveal>
        <Reveal delay={280} className="mx-auto mt-5 grid max-w-[35rem] gap-x-7 gap-y-2.5 text-start text-[13px] font-medium text-[#6f685e] min-[520px]:grid-cols-2 min-[940px]:mx-0">{heroPoints.map((point) => <span key={point} className="inline-flex items-center gap-2"><BadgeCheck className="h-4 w-4 shrink-0 text-[#cf6547]" />{point}</span>)}</Reveal>
      </div>
    ...
    <section className="px-5 pb-20 pt-2 sm:px-8 min-[940px]:pb-28"><div className="mx-auto max-w-[1240px]">
      ...
      <div className="journal-story-scroller">{heroStories.map((story, index) => <Reveal key={story.title} delay={index * 70} className="journal-story-card"><div className="relative flex min-h-[355px] items-end overflow-hidden rounded-[26px] bg-[#e8dece] px-5 pt-7"><Image src={story.image} alt={story.alt} width={1250} height={2700} className="mx-auto w-[155px] translate-y-12 drop-shadow-[0_24px_30px_rgba(59,43,27,0.2)]" /></div><div className="space-y-5 px-1 pt-6"><div><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#bf5b40]">{story.label}</p><h3 className="mt-2 text-xl font-semibold tracking-[-0.025em]">{story.title}</h3><p className="mt-2 text-sm leading-6 text-[#71695f]">{story.description}</p></div><div className="rounded-2xl bg-white/54 p-4"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#8c8276]">{story.secondLabel}</p><p className="mt-1.5 text-sm font-bold">{story.secondTitle}</p><p className="mt-1 text-xs leading-5 text-[#71695f]">{story.secondDescription}</p></div></div></Reveal>)}</div>
    </div></section>

    <section id="how-it-works" className="bg-[#29251f] px-5 py-24 text-[#f7f2e9] sm:px-8 min-[940px]:py-32"><div className="mx-auto max-w-[1240px]">
      ...
      <div className="mt-16 grid gap-5 min-[860px]:grid-cols-3">{steps.map((step, index) => { const Icon = step.icon; return <Reveal key={step.title} delay={index * 80} as="article" className="rounded-[28px] bg-white/[0.055] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] sm:p-7"><div className="relative flex h-[310px] items-start justify-center overflow-hidden rounded-[22px] bg-[#39332c] pt-7"><Image src={step.image} alt={step.title} width={1250} height={2700} className="relative w-[164px] drop-shadow-[0_24px_32px_rgba(0,0,0,0.34)]" /></div><div className="mt-7 flex items-center justify-between"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-[#db6947] text-white"><Icon className="h-5 w-5" /></div><span className="font-mono text-xs text-white/35">0{index + 1}</span></div><h3 className="mt-8 text-2xl font-semibold tracking-[-0.035em]">{step.title}</h3><p className="mt-3 leading-7 text-[#b4ab9f]">{step.description}</p></Reveal> })}</div>
    </div></section>

    <section id="features" className="px-5 py-24 sm:px-8 min-[940px]:py-36"><div className="mx-auto max-w-[1240px]">
      <Reveal className="mx-auto max-w-4xl text-center"><Eyebrow><Zap className="h-3.5 w-3.5" /> Everything You Need</Eyebrow><h2 className="mt-6 text-5xl font-semibold leading-[0.98] tracking-[-0.065em] sm:text-7xl">Everything you need in a travel organizer app.</h2><p className="mx-auto mt-7 max-w-3xl text-lg leading-8 text-[#6f675d]">Master every booking in your itinerary, from flights and stays to maps, expenses, cancellation deadlines, and travel documents.</p></Reveal>
      <div className="mt-16 grid gap-5 min-[680px]:grid-cols-2 min-[980px]:grid-cols-12">{features.map((feature, index) => { const Icon = feature.icon; return <Reveal key={feature.title} delay={(index % 3) * 70} as="article" className={`journal-feature-card ${feature.layout}`}><div className="relative flex h-[270px] items-start justify-center overflow-hidden rounded-[24px] bg-[#e7ddcd] pt-7">{feature.images.length === 2 ? <><Image src={feature.images[0]} alt={feature.imageAlts[0]} width={1250} height={2700} className="journal-feature-phone journal-feature-phone-import" /><Image src={feature.images[1]} alt={feature.imageAlts[1]} width={1250} height={2700} className="journal-feature-phone journal-feature-phone-draft" /></> : <Image src={feature.images[0]} alt={feature.imageAlts[0]} width={1250} height={2700} className="w-[165px] drop-shadow-[0_22px_28px_rgba(68,50,32,0.2)]" />}</div><div className="px-1 pt-7"><div className="grid h-10 w-10 place-items-center rounded-2xl bg-[#f0d7ca] text-[#bd573d]"><Icon className="h-4.5 w-4.5" /></div><h3 className="mt-6 text-2xl font-semibold tracking-[-0.035em]">{feature.title}</h3><p className="mt-3 max-w-xl leading-7 text-[#71695f]">{feature.description}</p></div></Reveal> })}</div>
    ...
    </div><div className="journal-screen-rail mt-14" aria-label="TripCache app workflows">{mobileScreens.map(([src, alt], index) => <Reveal key={alt} delay={(index % 5) * 45} className="shrink-0"><Image src={src} alt={alt} width={1250} height={2700} className={`h-auto w-[168px] drop-shadow-[0_26px_30px_rgba(64,48,31,0.2)] sm:w-[205px] ${index % 2 ? "translate-y-10" : ""}`} /></Reveal>)}</div></section>

    <section className="bg-[#eee7dc] px-5 py-24 sm:px-8 min-[940px]:py-36"><div className="mx-auto max-w-[1240px]">
      <Reveal className="mx-auto mb-20 max-w-3xl text-center min-[940px]:mb-32"><Eyebrow>Inside every trip</Eyebrow><h2 className="mt-6 text-4xl font-semibold leading-[1] tracking-[-0.055em] sm:text-6xl">Track deadlines, documents, flights, and expenses.</h2><p className="mt-6 text-lg leading-8 text-[#70685e]">Dive deeper into the trip details that usually get lost across inboxes, screenshots, and booking apps.</p><p className="mx-auto mt-5 max-w-xl text-sm leading-6 text-[#746c61]">Explore each workflow as you scroll. Every detail stays attached to the trip where it belongs.</p></Reveal>
      <div className="space-y-24 min-[940px]:space-y-36">{showcases.map((item, index) => { const Icon = item.icon; return <article key={item.id} id={item.id} className="grid items-center gap-12 min-[860px]:grid-cols-2 min-[860px]:gap-16"><Reveal className={index % 2 ? "min-[860px]:order-2" : ""}><div className="inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.17em] text-[#b9543a]"><Icon className="h-4.5 w-4.5" />{item.subtitle}</div><h3 className="mt-6 text-4xl font-semibold leading-[1] tracking-[-0.055em] sm:text-6xl">{item.title}</h3><p className="mt-7 max-w-xl text-lg leading-8 text-[#6f675d]">{item.description}</p></Reveal><Reveal delay={110} className={`relative mx-auto flex min-h-[530px] w-full max-w-[560px] items-center justify-center rounded-[34px] bg-[#e3d6c3] px-8 ${index % 2 ? "min-[860px]:order-1" : ""}`}><div className="absolute inset-[12%] rounded-full border border-[#3f352a]/10" /><Image src={item.image} alt={item.title} width={1250} height={2700} className="relative z-10 h-auto w-[240px] drop-shadow-[0_38px_42px_rgba(61,44,28,0.24)] sm:w-[285px]" /></Reveal></article> })}</div>
    </div></section>
    ...
    <section className="bg-[#29251f] px-5 py-24 text-[#f7f2e9] sm:px-8 min-[940px]:py-32"><div className="mx-auto max-w-[1240px]">
      ...
      <div className="mx-auto mt-16 grid max-w-5xl gap-x-12 gap-y-14 min-[720px]:grid-cols-2">{postBookingUseCases.map((useCase, index) => { const Icon = useCase.icon; return <Reveal key={useCase.title} delay={(index % 2) * 70} as="article" className="grid grid-cols-[auto_1fr] gap-5"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#db6947] text-white"><Icon className="h-5 w-5" /></div><div><h3 className="text-xl font-semibold tracking-[-0.025em]">{useCase.title}</h3><p className="mt-3 leading-7 text-[#b4ab9f]">{useCase.description}</p></div></Reveal> })}</div>
    </div></section>

    <section id="faq" className="journal-faq px-5 py-20 sm:px-8 min-[940px]:py-28"><div className="mx-auto max-w-[1080px]">
      <Reveal className="max-w-[820px]"><Eyebrow>FAQ</Eyebrow><h2 className="mt-6 text-4xl font-semibold tracking-[-0.055em] sm:text-6xl">Frequently asked questions</h2><p className="mt-5 text-lg leading-8 text-[#6f675d]">Straight answers about pricing, availability, email import, and the post-booking workflow.</p></Reveal>
      <div className="mt-14 space-y-3">{faqs.map((faq, index) => <Reveal key={faq.question} delay={(index % 3) * 45}><details open={index === 0} className="group rounded-[22px] bg-white/46 px-6 shadow-[0_1px_0_rgba(255,255,255,0.7),0_16px_44px_rgba(72,53,33,0.055)] sm:px-8"><summary className="flex cursor-pointer list-none items-center justify-between gap-6 py-6 text-lg font-semibold marker:content-none sm:text-xl"><span>{faq.question}</span><span aria-hidden="true" className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[#eadfce] text-xl font-normal text-[#b9543a] transition-transform duration-150 group-open:rotate-45">+</span></summary><p className="max-w-3xl pb-7 leading-7 text-[#71695f]">{faq.answer}</p></details></Reveal>)}</div>
      <Reveal className="mt-14 rounded-[30px] bg-[#db6947] p-8 text-center text-white sm:p-12"><h3 className="text-2xl font-semibold">Still have questions?</h3><p className="mt-2 text-white/76">Contact the TripCache support team for setup or billing help.</p><Link href="mailto:support@trip-cache.com" className="design-one-press mt-7 inline-flex min-h-12 items-center justify-center rounded-full bg-white px-7 font-semibold text-[#29251f] shadow-[0_12px_28px_rgba(98,41,25,0.2)]">Contact support</Link></Reveal>
    </div></section>
